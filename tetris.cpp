...
* Дата:      18.12.2023                          *
* Название:  Тетрис                              *
*   https://onlinegdb.com/B3tr3sY5vK             *
**************************************************/
#include <iostream>
#include <windows.h>
#include <cstdlib>
...
const int scr_width = fld_width * 2; // размер консольного окна
...
int TFigure::Check() {
  CalcCoord();
  for (int i = 0; i < coordCnt; ++i) // не даем выйти за вертикальные поля
    if (coord[i].X < 0 || coord[i].X >= fld_width)
      return 1;
  for (int i = 0; i < coordCnt; ++i) // оставляем внизу горизонтального поля
