    ...
    for (int matrixIndexX = 0; matrixIndexX < width; ++matrixIndexX) {
      if (matrixIndexX == 0 || matrixIndexX == width - 1) {
        cout << "#";
          ...
          for (int matrixIndex = 0; matrixIndex < nTail; ++matrixIndex) { // вывод хвоста
            if (matrixIndexY == tailY[matrixIndex] && matrixIndexX == tailX[matrixIndex]) {
              cout << "o";
              print = true;
            }
          } if (!print) {
...
void logic() {
  ...
  for (int matrixIndex = 1; matrixIndex < nTail; ++matrixIndex) {
    prev2X = tailX[matrixIndex];
    prev2Y = tailY[matrixIndex];
    tailX[matrixIndex] = prevX;
    tailY[matrixIndex] = prevY;
    prevX = prev2X;
    prevY = prev2Y;
  }
  ...
  for (int matrixIndex = 0; matrixIndex < nTail; ++matrixIndex) {
    if (x == tailX[matrixIndex] && y == tailY[matrixIndex]) {
      gameOver = true;
    }
  }
